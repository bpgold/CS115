...
def insertV1(L, i):
    '''inserts an element at index i in a partly sorted list of L[0:i+1]'''
    ...
    while i > 0 and L[i - 1] > L[i]:
        # Swap elements at indices i - 1 and i(to the right)
        swap(L, i-1, i)
        i -= 1
...
def search(L, i, x):
    ''' L - The sorted list. i- The length of the sorted portion of the list.
    x-The value to search for.'''

    '''Returns the index where x should be inserted in the sorted portion.'''
    ...
    j = 0
    while j < i and L[j] <= x:
        assert allLE(L[0:j], x)  # invariant: L[0:j] <= x
        j += 1
    assert allLE(L[0:j], x)  #assertion after loop done
    return j
...
def insertV2(L, i):
    '''Modifies the list by shifting elements to the right to create
    space for the new element at index i. The element at index i is
    placed in its correct sorted position in the list.'''
    ...
    insert = L[i] #save
    # search function to find position for the value
    j = search(L, i, insert)
    # shift elements to make room for the value
...
def letterCounts(fname):
    '''Prints list of pairs (n, c) in descending order by counts.'''
    ...
    counts = {'non-letter': 0} # new dictionary to store letters and non-letters

    # Loop through letters to initialize counts as 0
    for letter in 'abcdefghijklmnopqrstuvwxyz':
        counts[letter] = 0

    with open(fname, 'r') as file: # Read file
        for line in file:
            for c in line: #check if the character is a letter
                if c.isalpha(): #I googled how to check for a char
                    c = c.lower()  #lowercase for consistency
                    counts[c] += 1
                else:
                    counts['non-letter'] += 1
    #Counts to a list of tuples (n, c) and remove 0s
    result = [(count, letter) for letter, count in counts.items() if count > 0]
    result.sort(reverse=True)# Sort the list by count
    print(result)
